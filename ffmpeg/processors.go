...
func (p *Processor) getAnimationGIFThumbnail(filename string, targetWidth uint) (string, string) {
	...
	return fileid, "image/gif"
}

func (p *Processor) getAnimationWebPThumbnail(filename string, targetWidth uint) (string, string) {
	hash := sha256.Sum256([]byte(filename))
	fileid := filepath.Join(p.tempDir, hex.EncodeToString(hash[:])+".webp")
